...
def log_accuracy_AC(cp, ground_truth, dataset, log_flag, SEED=1234):
    # Init clustering hyperparameters
    n_clusters = cp.getint('Hyperparameters', 'ClusterNum')
    cluster_init = cp.getint('Hyperparameters', 'ClusterInit')
    # KMeans model
...
def assign_labels(Y_pred, Y):
    ...
    ind = linear_assignment(w.max() - w)
    for i,j in ind:
        Y_pred[np.where(Y_pred == i)[0]] = j
    return Y_pred

def log_anomalyPRF(cp, ground_truth, dataset, log_flag, SEED=1234):
    # Init clustering hyperparameters
    n_clusters = cp.getint('Hyperparameters', 'ClusterNum')
    cluster_init = cp.getint('Hyperparameters', 'ClusterInit')
    # KMeans model
