...
def build_layer_loss(X, out, num, cp, scope):
    aeid = 'AE' + num
    ...
    with tf.name_scope(scope):
        ...
        # Optimizer
        if cp.get('Experiment', 'PREFIX') == 'WEATHERIID-ALLONE':
            ae_optimizer = tf.train.AdamOptimizer(learning_rate=1e-3)
        else:
...
def build_stacked_loss(X, out, cp, scope='sda_loss'):
    # Initialize loss
    with tf.name_scope(scope):
        ...
        X = reshape_3D(X, cp)
        out = reshape_3D(out, cp)
        sda_mse = 1 - tf.reduce_mean(tf.image.ssim(X, out, 1.0))

        # Optimizer
        if cp.get('Experiment', 'PREFIX') == 'WEATHERIID-ALLONE':
            sda_optimizer = tf.train.AdamOptimizer(learning_rate=1e-3)
        else:
...
def build_px(cp, ae_ids, SEED):
    ...
    # Loss
    sda_lr, sda_mse, sda_t_op = build_stacked_loss(sda_X, sda_out, cp)
...
def recon_loss(X, out, cp, scope):
    ...
    with tf.name_scope(scope):
        ...
        X = reshape_3D(X, cp)
        out = reshape_3D(out, cp)
        ae_mse = 1 - tf.reduce_mean(tf.image.ssim(X, out, 1.0))
...
def EviTRAM_loss(X, out, Z, Qs, ks, vl, cp, cmse=0.5, ccond=1.0):
    px_mse, px_lr, px_t_op = recon_loss(X, out, cp, 'RECN')
