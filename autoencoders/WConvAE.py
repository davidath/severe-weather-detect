...
def build_encoder(incoming, cp, SEED, scope='px_encoder', reuse=False):
    ...
    with tf.name_scope(scope):
        cae = incoming
        cae = tf.layers.dropout(cae, float(cp.get('Dropout', 'rate')), seed=SEED)
        ...
        try:
            ...
            cae = tf.reshape(
                incoming, shape=[-1, height * width * channels, frames])
            print cae.shape
        except:
...
def build_decoder(incoming, cp, SEED, last_shape,
    ...
    with tf.name_scope(scope):
        ...
        cae = tf.image.resize_images(cae, (cae.shape[1]*2, cae.shape[2]*2))
        print cae.shape
        cae = tf.reshape(cae, shape=[-1, cae.shape[1] * cae.shape[2], cae.shape[3]])
        print cae.shape
        cae_last = cae
        ...
        for sect in decon_lst:
            if sect == decon_lst[decon_num - 1]:
                ...
                filters = cp.getint(sect, 'Filters')
                cae = tf.contrib.nn.conv1d_transpose(cae,
                        tf.Variable(tf.random_normal([fsize, filters, cae.get_shape().as_list()[2]])),
                                         [batch_size, cae.get_shape().as_list()[1]*2, filters], 
...
def recon_loss(X, out, scope):
    ...
    with tf.name_scope(scope):
        ...
        out = tf.reshape(out, shape=[-1, 64, 64, 1])
...
def EviTRAM_loss(X, out, Z, Qs, ks, vl):
    ...
    with tf.name_scope('multi_cond'):
        ...
        for pos, Q in enumerate(Qs):
            ...
            multi_loss += cond_l
        multi_loss = tf.constant(1.00) * tf.reduce_mean(multi_loss)
...
def build_px(cp, SEED):
    ...
    conv_In = tf.placeholder(tf.float32,
                             shape=[None, cp.getint('Input', 'Width'), cp.getint('Input', 'Frames')],
                             name='conv_IN')
    # Building network
