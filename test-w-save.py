...
def px(pae_dict):
    ...
    with tf.Session(config=config) as sess:
        ...
        if cp.get('Experiment', 'PREFIX') == 'MNIST' or \
            ...
            batch_size=batch_size)
            if not(np.array_equal(XX_test, np.zeros(shape=(1,1)))):
                utils.save_OOM(sess, pae_dict['sda_in'], XX_test,
                pae_dict['sda_hidden'],
                path=cp.get('Experiment', 'PX_Z_TEST'),
                batch_size=batch_size)

                utils.save_OOM(sess, pae_dict['sda_in'], XX_test,
                pae_dict['sda_out'],
                path=cp.get('Experiment', 'PX_XREC_TEST'),
                batch_size=batch_size)
...
def evitram(evitramd):
    ...
    with tf.Session(config=config) as sess:
        ...
        if cp.get('Experiment', 'PREFIX') == 'MNIST' or \
            ...
            batch_size=batch_size)
            if not(np.array_equal(XX_test, np.zeros(shape=(1,1)))):
                utils.save_OOM(sess, evitram_dict['sda_in'], XX_test,
                evitram_dict['sda_hidden'],
                path=cp.get('Experiment', 'PX_Z_TEST'),
                batch_size=batch_size)

                utils.save_OOM(sess, evitram_dict['sda_in'], XX_test,
                evitram_dict['sda_out'],
                path=cp.get('Experiment', 'PX_XREC_TEST'),
                batch_size=batch_size)
